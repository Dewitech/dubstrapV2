...
jQuery.appendo = function() {
	...
	this.init = function(obj,opt) {
		...
		var $add_btn btn-default = jQuery('#form-child-add').click(clicked_add),
			$del_btn btn-default = new_button(options.labelDel).click(clicked_del).hide()
		;
		...
		function update_buttons(rowdelta)
		{
			...
			// $add_btn btn-default.attr('disabled',(!options.maxRows || (rows < options.maxRows))?false:true);
			// Show remove button if we've added rows and allowDelete is set
			(options.allowDelete && (rows > 1))? $del_btn btn-default.show(): $del_btn btn-default.hide();
		};
		...
		jQuery('<div />')
			...
			.append( $add_btn btn-default, $del_btn btn-default )
			.insertAfter(obj);
		*/
