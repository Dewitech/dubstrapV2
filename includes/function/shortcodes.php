...
function dubstrap_button( $atts, $content = null ) {
	...
	elseif($style =='button'){
	   return '<a href="'. $url.'" target="'.$target.'" class="btn btn-default btn btn-default-'.$size.'" title="'.$title.'">'.$content.'</a>';
	}
	elseif($style =='primary'){
	   return '<a href="'. $url.'" target="'.$target.'" class="btn btn-default btn btn-default-primary btn btn-default-'.$size.'" title="'.$title.'">'.$content.'</a>';
	}
	else{
		return '<a class="btn btn-default btn btn-default-'.$size.' btn btn-default-'.$type.' btn btn-default-'.$style.'" target="'.$target.'" href="'.$url.'" title="'.$title.'">' .do_shortcode($content). '</a>';
	}
}
...
function dubstrap_dropcap( $atts, $content = null ) {
	...
	if($type == 'quote'){
		return '<col-md- class="quote"></col-md->';
	}
	if($type == 'circleprimary'){
		return '<col-md- class="drop-cap circle primary">' . $content . '</col-md->';
	}
	else{
   		return '<col-md- class="drop-cap '. $type .' ">' . $content . '</col-md->';
	}
}
...
function dubstrap_shortcode_contactform ( $atts, $content = null ) {
		...
		if( isset( $emailSent ) && $emailSent == true ) {
			...
			$html .= '<col-md- class="has_sent hide"></col-md->' . "\n";
		}
		...
		if ( $email == '' ) {} else {
			...
			if ( $show_default_fields != 'no' ) {
				...
				if( array_key_exists( 'contactName', $error_messages ) ) {
					$html .= '<col-md- class="error">' . $error_messages['contactName'] . '</col-md->' . "\n";
				}
				...
				if( array_key_exists( 'contactEmail', $error_messages ) ) {
					$html .= '<col-md- class="error">' . $error_messages['contactEmail'] . '</col-md->' . "\n";
				}
				...
				if( array_key_exists( 'contactMessage', $error_messages ) ) {
					$html .= '<col-md- class="error">' . $error_messages['contactMessage'] . '</col-md->' . "\n";
				}
					...
					jQuery('form#contact-form-<?php echo $id;?>').submit(function() {
						...
						jQuery('form#contact-form-<?php echo $id;?> .requiredField').each(function() {
							if(jQuery.trim(jQuery(this).val()) == '') {
								...
								jQuery(this).parent().append('<div class="clear"></div><col-md- class="error"><?php _e('You forgot to enter your', 'framework'); ?> '+labelText+'</col-md->');
								...
								if(!emailReg.test(jQuery.trim(jQuery(this).val()))) {
									var labelText = jQuery(this).prev().text();
									jQuery(this).parent().append('<div class="clear"></div><col-md- class="error"><?php _e('You entered an invalid', 'framework'); ?> '+labelText+'</col-md->');
									jQuery(this).addClass('inputError');
									hasError = true;
								}
							}
				...
				foreach ( $formatted_dynamic_atts as $k => $v ) {
					...
					if ( substr( $k, 0, 6 ) == 'radio_' ) {
						...
						$html .= '<p><label for="' . $k . '">' . $v['label'] . '</label>' . "\n";
							...
							$html .= '<col-md- class="dubstrap-radio-container fl">' . "\n";
							...
							$html .= '</col-md-><!--/.dubstrap-radio-container-->' . "\n";
					}
